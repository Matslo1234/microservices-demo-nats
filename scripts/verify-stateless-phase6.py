...
def verify_manifests() -> str:
    ...
    nats_policy = document(
        release_nats, "NetworkPolicy", "nats-clients-and-cluster"
    )
    require(
        nats_policy,
        "- benchmark-runner",
    ...
    release_backup = document(release_nats, "CronJob", "nats-backup")
    forbid(release_backup, "suspend: true")
    benchmark_nats = run(
        [kubectl, "kustomize", "kubernetes-manifests/nats/benchmark"]
    )
    benchmark_backup = document(
        benchmark_nats, "CronJob", "nats-backup"
    )
...
def verify_release_manifests() -> None:
    ...
    single_replica = (
        ROOT
        / "release"
        / "kubernetes-manifests-single-replica-no-loadgenerator.yaml"
    )
    benchmarks = tuple(
        ROOT / "benchmark" / name
        for name in (
    ...
    single_replica_text = single_replica.read_text()
    require(
        no_load_text,
    ...
    for content in (full_text, no_load_text, single_replica_text):
        forbid(
            content,
        ...
        for application in APPLICATIONS:
            if application == "benchmarkservice":
                continue
        ...
        for name in ("redis-cart-cluster", "redis-checkout-cluster"):
            redis = document(content, "StatefulSet", name)
            require(redis, "--save 3600 1")
            forbid(redis, '--save ""')
    for path in benchmarks:
        ...
        redis_stateful_sets = {
            name: document(content, "StatefulSet", name)
            for name in (
                "redis-cart-cluster",
